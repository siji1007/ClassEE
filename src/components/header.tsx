...
const Header: React.FC = () => {
  ...
  return (
    <header className="fixed top-0 w-full z-50 flex items-center justify-between px-6 py-2 bg-gradient-to-r from-gray-700 to-red-700 text-white shadow-md">
      ...
      {role && (
        <button onClick={handleSignOut} className="absolute right-6 hover:bg-white hover:text-red-800 text-white font-semibold py-2 px-2 rounded-md transition-all text-sm border-2 b-white" >
          Sign Out
        </button>
      )}
    </header>
